# -*- coding: utf-8 -*-
"""Part_1_Jayant_Chaudhari_chaudharijay2000_gmail_com_Self_case_study_1_Predictive_maintenance_(With_feature_extraction).ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1FtQBdKqlTB6v4qzTKg7N-giPsKevtZyS
...
"""From this table, we can observe the following for the Random Forest model in comparison to Logistic Regression, Linear SVC, and Decision Tree:

*   **Accuracy**: The Random Forest model shows a slightly higher accuracy (0.9559) compared to Logistic Regression (0.9556) and Linear SVC (0.9556), and is on par with the Decision Tree (0.9559).

*   **Precision (Macro Avg)**: Random Forest has a precision of 0.55, which is marginally better than Logistic Regression and Linear SVC (both 0.54), and equal to the Decision Tree.

*   **Recall (Macro Avg)**: All models show very high and consistent recall (0.97).

*   **F1-Score (Macro Avg)**: Random Forest (0.63) performs slightly better than Logistic Regression and Linear SVC (both 0.62), and is tied with Decision Tree (0.63).

*   **ROC AUC (Macro Avg)**: All models, including Random Forest, achieve excellent macro-averaged ROC AUC scores (around 0.9920-0.9922). The Random Forest model has an ROC AUC of 0.9922, similar to Logistic Regression.

**Conclusion**: The Random Forest model, along with the Decision Tree, shows a marginal improvement in Accuracy, Precision, and F1-Score (Macro Avg) over Logistic Regression and Linear SVC. All models exhibit strong Recall and ROC AUC, suggesting they are generally good at distinguishing between classes. The Random Forest model is a strong performer among the evaluated models, particularly for this dataset where all models show very competitive results.
...
# Task
Okay, I will proceed with generating the comprehensive Results Section for your research paper.

Here is an outline of the Results Section, incorporating the details from the plan:
...
*   **Dataset Overview**: Briefly describe the initial datasets (telemetry, error, maintenance, machines, failures) and their characteristics (e.g., number of records, columns).
*   **Data Cleaning**: Detail steps taken for data cleaning, such as handling missing values and data type conversions.
*   **Time Series Characteristics**: Discuss the temporal nature of the telemetry data and the implications for feature engineering.

#### 4.2. Feature Engineering
*   **Lag Features (Telemetry)**: Explain the creation of 3-hour and 24-hour lag features (mean and standard deviation) from telemetry data (volt, rotate, pressure, vibration). Provide rationale for selecting these windows.
*   **Lag Features (Errors)**: Describe how error data was transformed into count-based features (e.g., `error1count`, `error2count`) over 24-hour periods, emphasizing the categorical nature of error IDs.
*   **Days Since Last Replacement (Maintenance)**: Detail the process of calculating the "days since last component replacement" for each component (`comp1`, `comp2`, `comp3`, `comp4`) using the maintenance records.
*   **Machine Features**: Explain the inclusion of static machine features (model, age) and the one-hot encoding of the 'model' feature.
*   **Final Feature Matrix**: Summarize the composition of the `final_dataset`, including its dimensions and the types of features generated.

#### 4.3. Label Construction and Data Imbalance Handling
*   **Label Definition**: Explain the construction of the target variable ('failure'), defining a 24-hour prediction window for component failures (comp1, comp2, comp3, comp4) versus no failure (none).
*   **Initial Label Distribution**: Present the initial distribution of the 'failure' labels, highlighting the significant class imbalance observed (e.g., "none" vs. specific component failures).
*   **SMOTE Application**: Describe the application of the SMOTE technique to address data imbalance in the training set (`X_train_res`, `y_train_res`), and present the resulting balanced class distribution.

#### 4.4. Feature Selection
*   **Methodology**: Describe the univariate feature selection process using `SelectKBest` with the `chi2` scoring function and `MinMaxScaler`.
*   **Selected Features**: Present the top 5 selected features, listing them and discussing their scores or relative importance as indicated by the `SelectKBest` output.
*   **Impact of Selection**: Briefly comment on how feature selection aims to improve model performance and interpretability.

#### 4.5. Model Performance Evaluation
*   **Models Evaluated**: List the machine learning models trained and evaluated: Logistic Regression, Linear SVC, Decision Tree, and Random Forest.
*   **Performance Metrics Table**: Present the `results_df` table (Accuracy, Precision (Macro Avg), Recall (Macro Avg), F1-Score (Macro Avg), ROC AUC (Macro Avg)) for all models.
*   **Overall Accuracy and Macro-Averages**: Discuss the overall accuracy of the models, noting the high scores due to class imbalance. Emphasize the importance of macro-averaged metrics (Precision, Recall, F1-Score) for evaluating performance across minority classes.
*   **ROC AUC Analysis**: Highlight the macro-averaged ROC AUC scores for each model, interpreting their high values as indicators of good class separability.
*   **Comparison of Models**: Discuss the relative strengths and weaknesses of each model based on the presented metrics, specifically noting any models that show marginal improvements or distinct performance characteristics regarding the imbalanced classes.

#### 4.6. Confusion Matrix and ROC Curve Interpretation
*   **Confusion Matrices**: Interpret the normalized confusion matrices for each model (e.g., Logistic Regression, Linear SVC, Decision Tree, Random Forest). Focus on True Positives, False Positives, and False Negatives for each component failure class and the 'none' class, discussing trade-offs.
*   **Combined Macro-Average ROC Curves**: Analyze the combined macro-average ROC curves plot, comparing the curves and AUC values of all models. Discuss how these curves visually represent the models' ability to discriminate between classes, particularly in the context of the multi-class problem.

#### 4.7. Model Interpretability with SHAP (XGBoost example)
*   **Global Feature Importance**: Summarize the overall SHAP summary plot for the XGBoost model (even if XGBoost itself wasn't fully detailed in performance metrics, the SHAP analysis was performed). Explain what the plot reveals about the most impactful features (e.g., `volt_24H_mean`, `comp1`, `error2count`, `error3count`, `error5count`) across all classes.
*   **Class-Specific SHAP**: Interpret the class-specific SHAP summary plots (e.g., for class 0, class 1). Explain how these plots provide granular insights into which features contribute positively or negatively to the prediction of individual failure types.

#### 4.8. Conclusion of Results
*   **Key Findings**: Summarize the main findings from the model evaluation, feature importance, and interpretability analyses.
*   **Best Performing Models**: Identify the best-performing models based on a holistic view of the metrics, especially considering the macro-averaged scores and ROC AUC.
*   **Practical Implications**: Discuss the practical implications of these results for predictive maintenance, relating model performance and interpretability back to the business objectives of preventing costly downtime.
*   **Meeting Objectives**: Conclude whether the models, particularly the best ones, adequately address the problem statement and business objectives.

I will now proceed with generating the content for this Results Section.

## Outline Results Section

### Subtask:
Create an outline for the results section, detailing the subsections and key points to be covered, such as dataset description, feature engineering methods, handling data imbalance, feature selection findings, and the performance and interpretability of each machine learning model.
...
#### Instructions
The Results Section will be structured as follows:
...
## Describe Data Preprocessing and Feature Engineering

### Subtask:
Detail the steps taken for data preprocessing and feature engineering, including how telemetry, error, maintenance, and machine features were extracted and combined. Explain the creation of lag features, error counts, and days since last component replacement. Also, explain the label construction process for component failures within a 24-hour window.

### 1. Dataset Overview and Initial Preprocessing

To begin, we examined each of the raw datasets used in this case study:

*   **Telemetry Data (`telemetry`):**
    *   **Dimensions:** (876100, 6), indicating 876,100 data points and 6 columns.
    *   **Key Attributes:** `datetime`, `machineID`, `volt`, `rotate`, `pressure`, and `vibration`.
    *   **Data Type Conversion:** The `datetime` column was initially an object type and was converted to a pandas datetime object using `pd.to_datetime(telemetry['datetime'], format="%Y-%m-%d %H:%M:%S")`.
    *   **Missing/Duplicate Values:** No missing values or duplicate records were found in this dataset.

*   **Errors Data (`errors`):**
    *   **Dimensions:** (3919, 3), with 3,919 data points and 3 columns.
    *   **Key Attributes:** `datetime`, `machineID`, and `errorID`.
    *   **Data Type Conversion:** The `datetime` column was converted to a pandas datetime object, and `errorID` was cast to a categorical type.
    *   **Missing/Duplicate Values:** No missing values or duplicate records were identified.

*   **Maintenance Data (`maint`):**
    *   **Dimensions:** (3286, 3), containing 3,286 data points and 3 columns.
    *   **Key Attributes:** `datetime`, `machineID`, and `comp`.
    *   **Data Type Conversion:** The `datetime` column was converted to a pandas datetime object, and `comp` was cast to a categorical type.
    *   **Missing/Duplicate Values:** No missing values or duplicate records were found.

*   **Machines Data (`machines`):**
    *   **Dimensions:** (100, 3), representing 100 machines and 3 columns.
    *   **Key Attributes:** `machineID`, `model`, and `age`.
    *   **Data Type Conversion:** The `model` column was converted to a categorical type.
    *   **Missing/Duplicate Values:** This dataset also contained no missing values or duplicate records.

*   **Failures Data (`failures`):**
    *   **Dimensions:** (761, 3), with 761 data points and 3 columns.
    *   **Key Attributes:** `datetime`, `machineID`, and `failure`.
    *   **Data Type Conversion:** The `datetime` column was converted to a pandas datetime object, and `failure` was cast to a categorical type.
    *   **Missing/Duplicate Values:** No missing values or duplicate records were present in this dataset.

### 2. Telemetry Feature Engineering

Telemetry data, being time-series in nature, was used to create lagging features that capture short-term and long-term trends in machine behavior.

*   **Short-Term Lag Features (3-hour window):**
    *   For each of the telemetry sensor readings (`volt`, `rotate`, `pressure`, `vibration`), the mean and standard deviation were calculated over a 3-hour rolling window.
    *   This was achieved by pivoting the `telemetry` data to have `datetime` as the index and `machineID` as columns, then resampling with a '3H' frequency and calculating the mean (`_3H_mean`) and standard deviation (`_3H_std`).

*   **Long-Term Lag Features (24-hour window):**
    *   Similarly, for each sensor reading, the mean and standard deviation were calculated over a 24-hour rolling window.
    *   These features were also resampled with a '3H' frequency to align with the short-term features, resulting in `_24H_mean` and `_24H_std` features.

*   **Combining Telemetry Features (`telemetry_feat`):**
    *   All calculated 3-hour and 24-hour mean and standard deviation features for `volt`, `rotate`, `pressure`, and `vibration` were concatenated. This combined DataFrame, named `telemetry_feat`, provides a comprehensive view of each machine's recent and past operational patterns, which is crucial for predictive maintenance.

### 3. Error Feature Engineering

The `errors` dataset contains categorical `errorID` values indicating different types of errors. To transform this into a usable format for our models, we created count-based features:

*   **One-Hot Encoding `errorID`:**
    *   The `errorID` column was one-hot encoded to create separate binary columns for each error type (e.g., `error1`, `error2`, `error3`, `error4`, `error5`). This resulted in a DataFrame where each row corresponds to a specific error event, and the respective `errorID` column is marked `True` (or `1`) while others are `False` (or `0`).

*   **Aggregating Error Counts:**
    *   Errors for a given machine at a specific timestamp (rounded to the hour, consistent with telemetry data) were combined by grouping by `machineID` and `datetime` and summing the one-hot encoded error type columns. This ensured that if multiple errors occurred for the same machine within the same hour, they were correctly aggregated.

*   **Creating Lagged Error Counts (`error_count_total`):**
    *   To capture the frequency of errors over a longer period, we calculated the total number of errors of each type over 24-hour rolling windows. This was done by applying a `rolling(window=24).sum()` operation to the aggregated error data, which was then resampled every 3 hours (`resample('3H', closed='left', label='right').first()`).
    *   The resulting DataFrame, `error_count_total`, includes columns like `error1count`, `error2count`, etc., representing the total occurrences of each error type within the preceding 24 hours, observed at 3-hour intervals. Missing values introduced during rolling or resampling were filled with 0.0.

### 4. Maintenance Feature Engineering

The `maint` dataset provides records of component replacements. To create meaningful features from this data, we focused on calculating the time elapsed since the last replacement for each component type. This approach is highly relevant as component degradation is often correlated with usage time:

*   **One-Hot Encoding Components (`comp_rep`):**
    *   Initially, the `comp` column in the `maint` dataset (e.g., `comp1`, `comp2`, `comp3`, `comp4`) was one-hot encoded. This created separate binary columns for each component type, indicating which component was replaced at a given `datetime`.

*   **Aggregating Component Replacements:**
    *   Similar to the error data, if multiple components were replaced for the same machine at the same `datetime`, these were summed up by grouping by `machineID` and `datetime`.

*   **Integrating with Telemetry Timestamps:**
    *   To align maintenance records with the telemetry data's time granularity, the `comp_rep` DataFrame was merged with the `telemetry` DataFrame's `datetime` and `machineID` columns. This ensured that all time points present in the telemetry data were also considered in the component replacement features, with non-matching entries filled with 0.

*   **Calculating Days Since Last Replacement:**
    *   For each component column (e.g., `comp1`), the presence of a replacement (`1`) was replaced with its `datetime`. Non-replacement entries (`0`) were set to `None`.
    *   A forward-fill (`ffill`) operation was then applied to propagate the `datetime` of the last component replacement forward in time for each machine. This means that for any given timestamp, we knew the `datetime` of when each component was last replaced.
    *   Finally, the difference between the current `datetime` and the `datetime` of the last replacement was calculated and converted into 'days' using `np.timedelta64(1, "D")`. This yielded features representing the 'days since last component replacement' for `comp1`, `comp2`, `comp3`, and `comp4`. Data before '2015-01-01' was removed to ensure consistent historical tracking.

### 5. Machine Features and Final Dataset Assembly

The static characteristics of the machines, provided in the `machines` dataset, offer important contextual information:

*   **Machine Features Inclusion:**
    *   The `machines` dataset contains `machineID`, `model` (categorical type), and `age` (numerical, representing years in service). These features were directly integrated into our feature set.

*   **One-Hot Encoding `model`:**
    *   The `model` column, being categorical (`model1`, `model2`, `model3`, `model4`), was one-hot encoded using `pd.get_dummies()`. This transformed the single `model` column into multiple binary columns (e.g., `model1`, `model2`, `model3`, `model4`), making it suitable for machine learning models.

*   **Final Dataset Assembly (`final_features`):**
    *   All the previously engineered feature sets were sequentially merged to create a comprehensive `final_features` DataFrame. This involved merging `telemetry_feat`, `error_count_total`, `comp_rep` (days since last replacement), and the `machines` data (including the one-hot encoded `model` features) based on `datetime` and `machineID`.
    *   The merging was done with a 'left' or 'left' join where appropriate to ensure all time points from the telemetry features were preserved, and non-matching entries were filled with 0 where logical (e.g., for error counts).
    *   After this comprehensive merge, the `final_features` DataFrame had a shape of (291300, 29), representing 291,300 data points and 29 features, with no missing or duplicate values remaining.

### 6. Label Construction

The most critical step in preparing the data for predictive modeling is constructing the target variable, which in this case is the 'failure' label. This process aims to identify periods leading up to a component failure within a specific time window.

*   **Merging with `failures` Data:**
    *   The comprehensive `final_features` DataFrame was merged with the `failures` dataset using a left merge on `datetime` and `machineID`. This brought the `failure` column (indicating the failed component, e.g., 'comp1', 'comp2', 'comp3', 'comp4') into the main feature set.

*   **Defining the Prediction Window (24-hour window):**
    *   The objective is to predict failures that occur in the *near future*. A 24-hour prediction window was chosen. This means that if a machine experienced a component failure, all feature records leading up to that failure within the preceding 24 hours should be labeled accordingly.

*   **Backfilling Failure Labels (`fillna(method='bfill', limit=7)`):**
    *   Initially, only the exact timestamps of failures had a corresponding component label. All other time points had `NaN` values in the `failure` column. To implement the 24-hour prediction window, the `fillna(method='bfill', limit=7)` method was applied.
    *   `'bfill'` (backward fill) propagates the failure label backwards in time. The `limit=7` argument is crucial here: since our telemetry features are sampled every 3 hours, a `limit` of 7 backward fills covers approximately 21 hours (7 * 3 hours), effectively capturing the desired 24-hour prediction window before a failure.
    *   For example, if `comp4` failed at 2015-01-05 06:00:00, all feature records for that machine from 2015-01-04 09:00:00 onwards (7 * 3-hour intervals before the failure) would be labeled `comp4`.

*   **Assigning 'none' for Non-Failure Periods:**
    *   Any remaining `NaN` values in the `failure` column (i.e., time points not within 24 hours of any component failure) were filled with the label 'none'. This indicates periods of normal operation.

*   **Casting to Categorical Type:**
    *   Finally, the `failure` column was converted to a categorical data type to optimize memory usage and prepare it for classification algorithms.
...
## Detail Data Imbalance Handling and Feature Selection

### Subtask:
Describe the data imbalance handling strategy using SMOTE and the feature selection methodology using SelectKBest, presenting the top selected features and their rationale.

### Data Imbalance Handling and Feature Selection

#### 1. Initial Class Imbalance

Upon inspecting the `final_dataset` target variable, `failure`, we observe a significant class imbalance. The majority of records fall under the `'none'` class, indicating normal operation, while component failure classes (`'comp1'`, `'comp2'`, `'comp3'`, `'comp4'`) represent a small minority of the data. This is a common characteristic in predictive maintenance datasets, as machine failures are rare events. The output of `pd.value_counts(final_dataset.failure)` revealed:
...
As evident, the `'none'` class has a count of 285,684, overwhelmingly dominating the failure classes, which have counts ranging from 968 to 1985. This severe imbalance can lead to models that perform well on the majority class but poorly on the minority classes, as they tend to optimize for overall accuracy. To mitigate this, a technique for handling class imbalance was necessary.

#### 2. Data Balancing with SMOTE

To address the severe class imbalance, the Synthetic Minority Over-sampling Technique (SMOTE) was applied to the training data (`X_train_res`, `y_train_res`). SMOTE works by creating synthetic samples from the minority classes. It does this by taking samples from the minority class, computing the k-nearest neighbors for these samples, and then generating new synthetic samples along the line segments connecting the minority sample to its k-nearest neighbors.

The goal of applying SMOTE was to prevent the model from becoming biased towards the majority 'none' class and to improve its ability to correctly identify the minority failure classes, thus enhancing metrics like recall and F1-score for these critical events.

Here's how the class distribution changed in the training set after applying SMOTE:
...
As seen, SMOTE successfully balanced the training dataset, bringing the count of each minority class up to the level of the majority class (182,837 samples each), which is crucial for training a robust classification model.

#### 3. Feature Selection with `SelectKBest` and `chi2`

To identify the most relevant features and potentially improve model performance, univariate feature selection was performed using `SelectKBest` with the `chi2` (chi-squared) scoring function. The `chi2` statistic measures the dependence between stochastic variables, and in feature selection, it evaluates the significance of each feature with respect to the target variable.

**Necessity of `MinMaxScaler`:**

It is important to note that the `chi2` scoring function, used by `SelectKBest`, requires input features to be non-negative. Many real-world datasets, especially after certain transformations or feature engineering steps, can contain negative values. To ensure compatibility with `chi2`, the features were scaled using `MinMaxScaler` prior to applying `SelectKBest`. `MinMaxScaler` transforms features by scaling each feature to a given range, typically [0, 1]. This transformation ensures all feature values are non-negative, satisfying the requirement of the `chi2` statistic, and also helps in standardizing the data for other potential machine learning algorithms.

Specifically, `X_train`, `X_test`, and `X_cv` were scaled to `X_train_scaled`, `X_test_scaled`, and `X_cv_scaled` respectively, before passing them to `SelectKBest`.
...
#### 4. Top Selected Features from `SelectKBest`

After applying `SelectKBest` with the `chi2` scoring function to the training data (specifically, `X_train` for the `diccionario` creation), the following top 5 features were identified as having the highest scores, indicating their strong relevance to predicting machine failures:

1.  **`error5count`** (score: 82014.76)
2.  **`error3count`** (score: 56343.37)
3.  **`error2count`** (score: 46207.34)
4.  **`error1count`** (score: 32978.97)
5.  **`error4count`** (score: 30126.59)

All five of the top features are related to the count of different error types. This strongly suggests that the frequency and type of errors a machine experiences are critical indicators of impending failure. Intuitively, a higher number of specific error types within a given timeframe (24-hour rolling window in this case) would directly correlate with a higher probability of a component failure. These features directly capture the diagnostic signals that precede a breakdown, making them highly valuable for predictive maintenance models. The selection of these error count features highlights the importance of historical error logs in understanding and predicting machine health.

#### 5. Impact of Feature Selection

Feature selection, particularly with methods like `SelectKBest`, plays a crucial role in optimizing machine learning models for predictive maintenance. Its impact can be summarized as follows:

*   **Improved Model Performance**: By focusing on the most relevant features (in this case, `errorcount` features), the model can learn more effectively from meaningful patterns, leading to better predictive accuracy and generalization on unseen data. Irrelevant or redundant features can introduce noise and reduce the model's ability to discern true relationships.

*   **Reduced Overfitting**: A simpler model with fewer features is less likely to overfit the training data. This is particularly important in predictive maintenance where the goal is to make robust predictions across a variety of operating conditions and machine states.

*   **Enhanced Interpretability**: Using a reduced set of highly impactful features makes the model's decisions easier to understand and explain. When the top predictors are `errorcount` variables, it provides clear insights into the factors contributing to machine failure, which can be invaluable for maintenance planning and operational adjustments.

*   **Faster Training and Inference**: Training a model on fewer features requires less computational resources and time. This is beneficial for deploying models in real-time or near real-time predictive maintenance systems.

In this context, the selection of error count features directly aligns with the business problem of predicting component failures. The prominence of these features suggests that monitoring the frequency and type of errors is a highly effective strategy for anticipating machine downtime.
...
### Subtask:
Present the performance metrics for all models and discuss their implications.

**Reasoning**:
The subtask requires presenting the performance metrics for all trained models. The `results_df` DataFrame already contains these metrics, so the first step is to display this DataFrame.
"""
...
We trained and evaluated four machine learning models for predicting component failures: Logistic Regression, Linear SVC, Decision Tree, and Random Forest. The performance of these models on the test set, after applying SMOTE to balance the training data and using feature selection (SelectKBest with chi2), is summarized in the table below:
...
| :-------------------- | :------- | :-------------------- | :----------------- | :------------------- | :------------------ |
| Logistic Regression   | 0.9556   | 0.54                  | 0.97               | 0.62                 | 0.9922              |
| Linear SVC            | 0.9556   | 0.54                  | 0.97               | 0.62                 | 0.9920              |
| Decision Tree         | 0.9559   | 0.55                  | 0.97               | 0.63                 | 0.9920              |
| Random Forest         | 0.9559   | 0.55                  | 0.97               | 0.63                 | 0.9922              |


#### Overall Accuracy and Macro-Averages

As observed from the `results_df` table, all models achieved a high overall accuracy, ranging from approximately 95.56% to 95.59%. While these high accuracy scores might initially appear impressive, it is crucial to interpret them cautiously, especially in the context of highly imbalanced datasets characteristic of predictive maintenance. Given that the 'none' class (no failure) significantly outnumbers the failure classes, a model could achieve high accuracy by simply predicting 'none' most of the time. This highlights why overall accuracy alone can be misleading.

Therefore, we emphasize macro-averaged metrics (Precision, Recall, and F1-Score), which treat all classes equally, regardless of their support (number of true instances). These metrics provide a more robust evaluation of model performance across the minority failure classes, which are of critical business importance.

#### ROC AUC Analysis

The macro-averaged ROC AUC scores for all models are exceptionally high, hovering around 0.9920-0.9922. A high ROC AUC indicates that the models are very good at distinguishing between positive and negative classes (in a one-vs-rest fashion for multi-class classification). This suggests excellent class separability and the models' ability to rank instances correctly across the different failure types and the 'none' class.

#### Comparison of Models

Upon comparing the models:

*   **Logistic Regression and Linear SVC**: These models show similar performance across all metrics, with an accuracy of 0.9556, macro-averaged precision and F1-score of 0.54 and 0.62 respectively, and high ROC AUC scores. Their performance is solid, providing a good baseline.

*   **Decision Tree and Random Forest**: These ensemble models demonstrate a marginal but consistent improvement over Logistic Regression and Linear SVC. Both achieved a slightly higher accuracy of 0.9559, with macro-averaged precision and F1-score of 0.55 and 0.63, respectively. The Random Forest model also recorded the highest macro-averaged ROC AUC at 0.9922, tying with Logistic Regression. The Decision Tree's performance is notably strong for its simplicity, suggesting that the selected features capture clear decision boundaries.

In summary, while all models performed well due to the effectiveness of the engineered features and SMOTE application, the Decision Tree and Random Forest models show a slight edge in macro-averaged precision and F1-score, indicating a marginally better balance between identifying failures and avoiding false alarms across all classes.

#### 4.6.1. Logistic Regression Confusion Matrix

The normalized confusion matrix for the Logistic Regression model is displayed below. This matrix helps visualize the performance of the classification model by showing the number of true positive, true negative, false positive, and false negative predictions. In a multi-class context, the diagonal elements represent the correctly classified instances for each class (True Positives), while off-diagonal elements indicate misclassifications.

**Reasoning**:
I need to display the normalized confusion matrix for the Logistic Regression model based on the results from `log_reg_grid_results` to allow for interpretation as per the subtask instructions.
"""

plt.figure(figsize=(8,8))
plt.grid(visible=False)
plot_confusion_matrix(log_reg_grid_results['confusion_matrix'], classes=labels, normalize=True, title='Normalized Confusion Matrix - Logistic Regression', cmap=plt.cm.Greens)
plt.show()
