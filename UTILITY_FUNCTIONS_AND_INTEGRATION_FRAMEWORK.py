...
================================================================================
SECTION 5.5: TROUBLESHOOTING GUIDE
================================================================================
...
1. MACHINE-LEVEL GROUP K-FOLD VALIDATION
   ...
   Issue: Machine overlap != 0
   Solution:
   - Check machine_ids for duplicates
   - Ensure n_machines >= n_splits
   - Verify group column format
   
   Issue: F1 degradation > 10%
   Solution:
   - Indicates possible data issues
   - Check for temporal patterns
   - Verify train/test independence
   
   Issue: Very small folds
   Solution:
   - Reduce n_splits if n_machines is small
   - Ensure minimum 5-10 machines per fold
   - Use stratified sampling within groups
...
2. SMOTE APPLICATION
   ...
   - Use smaller sampling_strategy
   - Process in batches
   
   Issue: SMOTE creates unrealistic samples
   Solution:
   - Reduce k_neighbors (try k=3 or k=4)
   - Increase random_state for consistency
   - Review generated samples
   
   Issue: No improvement after SMOTE
   Solution:
   - Check class distribution
   - Verify SMOTE applied only to training
   - Try alternative: ADASYN or class weighting

3. THRESHOLD TUNING
   
   Issue: Optimal threshold = 0.0 or 1.0
   Solution:
   - Check class balance
   - Verify probability calibration
   - Review positive class ratio
   
   Issue: Threshold sensitive to test set
   Solution:
   - Tune on validation set instead
   - Use cross-validation for stability
   - Document threshold uncertainty
   
   Issue: Recall too low with tuned threshold
   Solution:
   - Use precision-recall curves
   - Find threshold with acceptable trade-off
   - Consider business requirements

4. GENERAL PIPELINE ISSUES
   
   Issue: Low overall performance
   Solution:
   - Check feature engineering
   - Verify data quality
   - Review class imbalance severity
   ...
   - Use fixed SMOTE seed (2)
   - Disable shuffle in preprocessing
   
   Issue: Production model differs from training
   Solution:
   - Save preprocessing objects (scaler)
   - Store exact hyperparameters
   - Version control configuration files
   - Test on production-like data
"""

print("\n" + "="*70)
print("✅ ALL UTILITY FUNCTIONS AND INTEGRATION TOOLS READY")
print("="*70)
print("\nAvailable Classes:")
print("  • ModelEvaluator: Comprehensive evaluation metrics")
print("  • ThresholdAnalyzer: Threshold optimization")
print("  • ModelComparison: Compare multiple models")
print("  • PdMPipeline: Complete integration wrapper")
print("\nKey Features:")
print("  ✓ Machine-Level Group K-Fold Validation")
print("  ✓ SMOTE Application with reproducibility")
print("  ✓ Threshold Tuning for rare classes")
